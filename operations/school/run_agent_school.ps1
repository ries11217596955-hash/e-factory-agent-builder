...
$cleanupRemoved=@()
$chunks=@()
$totalFactoryCandidates=0
$totalReadyAtoms=0
$totalStreamQuarantined=0
$lastProof=$null
$lastUseProof=$null
$chunkIndex=0
...
while($remaining -gt 0){
  ...
  $factoryOut=@(& powershell -NoProfile -ExecutionPolicy Bypass -File operations/school/curriculum/candidate_factory/generate_codex_curriculum_candidate_factory_run_v1.ps1 -TargetAccepted $chunkTarget -RunKind Test -BatchSize $batchSize -RunId $chunkRunId *>&1 | ForEach-Object {[string]$_})
  ...
  if($RunKind -eq 'Test'){
    ...
    $chunks += [ordered]@{chunk_index=$chunkIndex; chunk_target=$chunkTarget; inner_batch_size=$batchSize; factory_candidates=[int]$factoryReport.candidates_created; ready_atoms=[int]$stream.ready_atoms_total; digested=$false; recall_use=$false; cleanup_after_chunk=$true}
  ...
  $chunks += [ordered]@{
    ...
    inner_batch_size=$batchSize
    factory_candidates=[int]$factoryReport.candidates_created
    ready_atoms=[int]$stream.ready_atoms_total
    digested=$true
    digested_cells=[int]$pipeProof.digested_cells
    merged_count=[int]$pipeProof.merged_count
    cumulative_memory_merge=$pipeProof.cumulative_memory_merge
    existing_memory_seeded=$pipeProof.existing_memory_seeded
    existing_memory_cells_before=[int]$pipeProof.existing_memory_cells_before
    total_memory_bytes=[int]$pipeProof.total_memory_bytes
    recall_use_status=$useProof.status
    behavior_delta=$useProof.behavior_delta
    used_memory_cells_count=@($useProof.used_labels).Count
    cleanup_after_chunk=$true
  }
  $lastProof=$pipeProof
  $lastUseProof=$useProof
...
$base=[ordered]@{
  schema='agent_school_canonical_run_v6_chunked_cumulative'
  run_id=$runId
  run_kind=$RunKind
  target_accepted=$TargetAccepted
  outer_chunk_size=$outerChunkSize
  inner_batch_size_max=$innerBatchSizeMax
  chunk_count=@($chunks).Count
  chunks=@($chunks)
  runtime_ready=$false
  raw_route_absorption_allowed=$false
  factory_candidates_created=$totalFactoryCandidates
  ready_atoms=$totalReadyAtoms
  stream_quarantined=$totalStreamQuarantined
  ...
  retention_policy='KEEP_ACTIVE_COMPACT_MEMORY_AND_CANONICAL_PROOF_ONLY_V1'
  cleanup_removed=@($cleanupRemoved | Select-Object -Unique)
  cleanup_after_each_chunk=$true
  law='TargetAccepted + RunKind uses outer chunks of 5000 and inner factory batches of 100. Real uses cumulative compact semantic memory and cannot continue past a chunk without recall/use behavior_delta proof.'
}
if($RunKind -eq 'Test'){
  ...
  Write-Host "CHUNK_COUNT=$(@($chunks).Count)"
...
$base.pipeline_status=$lastProof.status
$base.validation_tier=$lastProof.selected_validation_tier
$base.digested_cells=[int]$lastProof.digested_cells
$base.merged_count=[int]$lastProof.merged_count
$base.cumulative_memory_merge=$lastProof.cumulative_memory_merge
$base.existing_memory_seeded=$lastProof.existing_memory_seeded
$base.existing_memory_cells_before=[int]$lastProof.existing_memory_cells_before
$base.raw_source_dependency_removed=$lastProof.raw_source_dependency_removed
$base.staged_raw_deleted=$lastProof.staged_raw_deleted
$base.normalized_digest_input_deleted=$lastProof.normalized_digest_input_deleted
$base.total_memory_bytes=[int]$lastProof.total_memory_bytes
$base.memory_root=$lastProof.memory_root
$base.recall_use_status=$lastUseProof.status
$base.used_memory_cells=@($lastUseProof.used_labels)
$base.baseline_decision=$lastUseProof.baseline_decision
$base.active_decision=$lastUseProof.active_decision
$base.behavior_delta=$lastUseProof.behavior_delta
$base.behavior_delta_definition=$lastUseProof.behavior_delta_definition
$base.boundary='Real is chunked and cumulative: each chunk digests into active compact memory seeded from prior active memory, proves recall/use behavior_delta, then removes transient traces before next chunk.'
WriteJson $proofPath $base 100
...
Write-Host "EXISTING_MEMORY_SEEDED=$($base.existing_memory_seeded)"
Write-Host "EXISTING_MEMORY_CELLS_BEFORE=$($base.existing_memory_cells_before)"
...
Write-Host "CLEANUP_AFTER_EACH_CHUNK=true"
