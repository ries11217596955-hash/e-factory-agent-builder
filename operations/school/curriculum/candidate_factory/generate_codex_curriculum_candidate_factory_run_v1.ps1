...
while($ordinal -lt $TargetAccepted){
  ...
  $checkpoint=[pscustomObject]@{schema='codex_candidate_factory_checkpoint_v1'; status='RUNNING'; run_id=$RunId; run_kind=$RunKind; run_slug=$runSlug; use_factory_memory=$UseFactoryMemory; use_topic_cursor=$UseTopicCursor; target_accepted=$TargetAccepted; batch_size=$BatchSize; batches_created=$created.Count; candidates_created=$ordinal; run_dir=$runDir; all_candidates_path=$allPath; generated_learning_key_count=$generatedKeys.Keys.Count; generated_theme_count=$generatedThemes.Keys.Count; updated_at=(Get-Date).ToString('o')}
  WriteJson (Join-Path $runDir 'checkpoint.json') $checkpoint 50
}
$report=[pscustomObject]@{schema='codex_candidate_factory_run_v1'; status='PASS_CODEX_CANDIDATE_FACTORY_GENERATION_V1'; runtime_ready=$false; run_id=$RunId; run_slug=$runSlug; run_kind=$RunKind; use_factory_memory=$UseFactoryMemory; use_topic_cursor=$UseTopicCursor; target_accepted=$TargetAccepted; batch_size=$BatchSize; batches_created=$created.Count; candidates_created=$TargetAccepted; run_dir=$runDir; all_candidates_path=$allPath; all_candidates_sha256=(Get-FileHash $allPath -Algorithm SHA256).Hash.ToLower(); generated_learning_key_count=$generatedKeys.Keys.Count; generated_theme_count=$generatedThemes.Keys.Count; codex_cli_invoked=$false; api_invoked=$false; active_memory_mutated=$false; batch_reports=@($created); boundary='Local cursor-guided factory generation only; no Codex CLI/API calls, no active promotion, no live proof.'}
