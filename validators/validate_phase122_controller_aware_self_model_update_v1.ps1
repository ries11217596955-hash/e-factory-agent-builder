...
$StepId = "PHASE122_BUILD_CONTROLLER_AWARE_SELF_MODEL_UPDATE_V1"
$NextAllowed = "PHASE123_RUN_CONTROLLER_GOVERNED_SELF_BUILD_TRIAL_V1"
$SelfModelPath = "self_model/BUILDER_SELF_MODEL.json"
$OutputPath = "self_build_batch/autonomy_trials/$StepId/CONTROLLER_AWARE_SELF_MODEL_UPDATE_OUTPUT.json"
...
foreach ($p in @("modules/invoke_controller_aware_self_model_update.ps1","modules/invoke_autonomous_loop_controller.ps1","orchestrator/run.ps1",$SelfModelPath,$OutputPath,$ResultPath,$ReportPath,$ProofPath)) { if (-not (Test-Path $p)) { Mark-Fail "MISSING=$p" } else { Write-Output "EXISTS=$p" } }
try {
  ...
  if ($SelfModel.controller_aware -ne $true) { Mark-Fail "SELF_MODEL_NOT_CONTROLLER_AWARE" }
  if ($closed -notcontains "NEED_AUTONOMOUS_LOOP_CONTROLLER") { Mark-Fail "CLOSED_NEED_MISSING_NEED_AUTONOMOUS_LOOP_CONTROLLER" }
  if ($SelfModel.current_detected_need -ne "NEED_CONTROLLER_GOVERNED_SELF_BUILD_TRIAL") { Mark-Fail "SELF_MODEL_NEED_UNEXPECTED=$($SelfModel.current_detected_need)" }
  ...
  if ($Output.closed_need -ne "NEED_AUTONOMOUS_LOOP_CONTROLLER") { Mark-Fail "OUTPUT_CLOSED_NEED_UNEXPECTED=$($Output.closed_need)" }
...
} catch { Mark-Fail "CONTROLLER_AWARE_VALIDATE_PARSE_FAIL=$($_.Exception.Message)" }
...
if ($Ok -eq $true) { Write-Output "PHASE122_CONTROLLER_AWARE_SELF_MODEL_UPDATE_VALIDATE_RESULT=PASS" } else { Write-Output "PHASE122_CONTROLLER_AWARE_SELF_MODEL_UPDATE_VALIDATE_RESULT=FAIL" }
if ($Ok -ne $true) { throw "PHASE122 validation failed." }
