...
$StepId = "PHASE126_BUILD_TRIAL_AWARE_SELF_MODEL_ADVANCE_V1"
$NextAllowed = "PHASE127_BUILD_SELF_BUILD_OPERATION_CONTRACT_V1"

$SelfModelPath = "self_model/BUILDER_SELF_MODEL.json"
$OutputPath = "self_build_batch/autonomy_trials/$StepId/TRIAL_AWARE_SELF_MODEL_ADVANCE_OUTPUT.json"
...
try {
  ...
  if ($SelfModel.trial_aware -ne $true) { Mark-Fail "SELF_MODEL_NOT_TRIAL_AWARE" }
  if ($closed -notcontains "NEED_CONTROLLER_GOVERNED_SELF_BUILD_TRIAL") { Mark-Fail "CLOSED_NEED_MISSING_NEED_CONTROLLER_GOVERNED_SELF_BUILD_TRIAL" }
  if ($SelfModel.current_detected_need -ne "NEED_SELF_BUILD_OPERATION_CONTRACT") { Mark-Fail "SELF_MODEL_NEED_UNEXPECTED=$($SelfModel.current_detected_need)" }
  ...
  if ($Output.closed_need -ne "NEED_CONTROLLER_GOVERNED_SELF_BUILD_TRIAL") { Mark-Fail "OUTPUT_CLOSED_NEED_UNEXPECTED=$($Output.closed_need)" }
  ...
  Mark-Fail "TRIAL_AWARE_VALIDATE_PARSE_FAIL=$($_.Exception.Message)"
}
...
if ($Ok -eq $true) {
  Write-Output "PHASE126_TRIAL_AWARE_SELF_MODEL_ADVANCE_VALIDATE_RESULT=PASS"
} else {
  Write-Output "PHASE126_TRIAL_AWARE_SELF_MODEL_ADVANCE_VALIDATE_RESULT=FAIL"
}

if ($Ok -ne $true) {
  throw "PHASE126 validation failed."
}
