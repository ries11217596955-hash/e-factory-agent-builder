...
foreach($required in @('outerChunkSize=5000','innerBatchSizeMax=100','cumulative_memory_merge','cleanup_after_each_chunk','PASS_REAL_FACTORY_DIGEST_RECALL_USE_V1','behavior_delta')){ if($runnerText -notmatch [regex]::Escape($required)){ throw "RUNNER_CHUNKED_CUMULATIVE_CONTRACT_MISSING:$required" } }
...
try {
  ...
  if($testProof.schema -ne 'agent_school_canonical_run_v6_chunked_cumulative'){ throw 'TEST_BAD_SCHEMA' }
  if([int]$testProof.outer_chunk_size -ne 5000){ throw 'TEST_OUTER_CHUNK_BAD' }
  ...
  if($realProof.cumulative_memory_merge -ne $true){ throw 'REAL_CUMULATIVE_MEMORY_MERGE_NOT_PROVEN' }
  if($realProof.existing_memory_seeded -ne $true){ throw 'REAL_EXISTING_MEMORY_NOT_SEEDED' }
  if($realProof.cleanup_after_each_chunk -ne $true){ throw 'REAL_CLEANUP_AFTER_EACH_CHUNK_NOT_PROVEN' }
  ...
  if([int]$realProof.digested_cells -lt [int]$realProof.existing_memory_cells_before){ throw 'REAL_MEMORY_SHRANK_DURING_CUMULATIVE_MERGE' }
} finally {
...
Write-Host 'VALIDATION_PASS=AGENT_SCHOOL_CANONICAL_CHUNKED_CUMULATIVE_V1_VALID'
Write-Host "TEST_STATUS=$($testProof.status)"
Write-Host "TEST_CHUNK_COUNT=$($testProof.chunk_count)"
Write-Host "TEST_READY_ATOMS=$($testProof.ready_atoms)"
Write-Host "REAL_STATUS=$($realProof.status)"
Write-Host "REAL_CHUNK_COUNT=$($realProof.chunk_count)"
Write-Host "REAL_OUTER_CHUNK_SIZE=$($realProof.outer_chunk_size)"
Write-Host "REAL_INNER_BATCH_SIZE_MAX=$($realProof.inner_batch_size_max)"
Write-Host "REAL_READY_ATOMS=$($realProof.ready_atoms)"
Write-Host "REAL_DIGESTED_CELLS=$($realProof.digested_cells)"
Write-Host "REAL_EXISTING_MEMORY_CELLS_BEFORE=$($realProof.existing_memory_cells_before)"
Write-Host "REAL_CUMULATIVE_MEMORY_MERGE=$($realProof.cumulative_memory_merge)"
Write-Host "REAL_CLEANUP_AFTER_EACH_CHUNK=$($realProof.cleanup_after_each_chunk)"
