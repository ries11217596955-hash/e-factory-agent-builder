...
$StepId = "PHASE129_BUILD_OPERATION_CONTRACT_AWARE_SELF_MODEL_ADVANCE_V1"
$NextAllowed = "PHASE130_BUILD_SELF_BUILD_OPERATION_READINESS_GATE_V1"

$SelfModelPath = "self_model/BUILDER_SELF_MODEL.json"
$OutputPath = "self_build_batch/autonomy_trials/$StepId/OPERATION_CONTRACT_AWARE_SELF_MODEL_ADVANCE_OUTPUT.json"
...
try {
  ...
  if ($SelfModel.operation_contract_aware -ne $true) { Mark-Fail "SELF_MODEL_NOT_OPERATION_CONTRACT_AWARE" }
  if ($closed -notcontains "NEED_SELF_BUILD_OPERATION_CONTRACT") { Mark-Fail "CLOSED_NEED_MISSING_NEED_SELF_BUILD_OPERATION_CONTRACT" }
  if ($SelfModel.current_detected_need -ne "NEED_SELF_BUILD_OPERATION_READINESS_GATE") { Mark-Fail "SELF_MODEL_NEED_UNEXPECTED=$($SelfModel.current_detected_need)" }
  ...
  if ($Output.closed_need -ne "NEED_SELF_BUILD_OPERATION_CONTRACT") { Mark-Fail "OUTPUT_CLOSED_NEED_UNEXPECTED=$($Output.closed_need)" }
  ...
  Mark-Fail "OPERATION_CONTRACT_AWARE_VALIDATE_PARSE_FAIL=$($_.Exception.Message)"
}
...
if ($Ok -eq $true) {
  Write-Output "PHASE129_OPERATION_CONTRACT_AWARE_SELF_MODEL_ADVANCE_VALIDATE_RESULT=PASS"
} else {
  Write-Output "PHASE129_OPERATION_CONTRACT_AWARE_SELF_MODEL_ADVANCE_VALIDATE_RESULT=FAIL"
}

if ($Ok -ne $true) {
  throw "PHASE129 validation failed."
}
